  ...
  get:
    ...
    async ({ get }) => {
      ...
      if (address) {
        ...
        return await getContractQuery(query, "voter")
      }
    },
})
...
const govAddressVotersQuery = selector({
  ...
  get: async ({ get }) => {
    ...
    if (address) {
      ...
      return (await getContractQueries(document, "voters")) || {}
    }
...
export const missingRewardsQuery = selector({
  ...
  get: ({ get }) => {
    ...
    return MISSING_REWARDS.reduce((acc, id) => {
...
const missingRewardsState = atom({
...
/* helpers */
export const calcVotingRewards = (poll, voter) => {
