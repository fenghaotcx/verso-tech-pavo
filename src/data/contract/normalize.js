...
import { div, gt, times } from "../../utils/math"
import { PriceKey, BalanceKey, StakingKey } from "../../hooks/contractKeys"
import { useStore, useStoreLoadable } from "../../utils/loadable"
import { exchangeRatesQuery } from "../../page/native/exchange"
import { bankBalanceQuery } from "../../page/native/balance"
...
const nativeBalancesState = atom({
...
const lpStakableBalancesState = atom({
...
const lpStakedBalancesState = atom({
...
const slpStakedBalancesState = atom({
...
const lpRewardBalancesState = atom({
...
const slpRewardBalancesState = atom({
...
export const minCollateralRatioState = atom({
  key: "minCollateralRatioState",
  default: {},
})

export const multipliersQuery = selector({
  key: "multiplierRatio",
  get: ({ get }) => ({
    uusd: "1",
    ...dict(
...
export const multipliersState = atom({
  key: "multiplierState",
  default: {},
})
...
export const useFindBalance = () => {
  ...
  return {
    contents: (token) => dictionary[getBalanceKey(token)][token],
...
export const useFindStaking = () => {
  ...
  return {
    contents: (key, token) => dictionary[key][token],
    isLoading: [
      lpStakableBalances,
      lpStakedBalances,
      slpStakedBalances,
...
/* helpers */
export const parsePairPool = (pool) => {
...
export const calcPairPrice = (param) => {
